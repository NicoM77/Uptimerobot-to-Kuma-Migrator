...
if (!UPTIME_ROBOT_API_KEY || !KUMA_URL || !KUMA_USERNAME || !KUMA_PASSWORD) {
  console.warn("Required env variables are missing!");
  console.warn('Copy ".env.sample" as ".env"');
  process.exit(1);
}

const UPTIME_ROBOT_GET_MONITOR_API_PATH =
...
const UPTIME_ROBOT_DISABLE_MONITOR_API_PATH =
...
const disableUptimeRobotMonitor = async (monitor: UptimeRobotMonitor) => {
  ...
  if (responseJson?.stat === "fail") {
    console.error(
      `Failed to edit monitor '${monitor.friendly_name}' due to '${responseJson.error?.type}'`
    );
    process.exit(1);
  } else {
...
const UPTIME_ROBOT_DELETE_MONITOR_API_PATH =
...
const deleteUptimeRobotMonitor = async (monitor: UptimeRobotMonitor) => {
  ...
  if (responseJson?.stat === "fail") {
    console.error(
      `Failed to delete monitor '${monitor.friendly_name}' due to '${responseJson.error?.type}'`
    );
    process.exit(1);
  } else {
...
const deleteKumaMonitorOnPage = async (page: Page) => {
  await page.waitForLoadState("domcontentloaded");
  await page.waitForLoadState("networkidle");

  await page.waitForTimeout(125);

  // Finde alle <a>-Elemente mit href, das mit "/dashboard/" beginnt
  ...
  const yesButton = await page.$(
    'button.btn.btn-danger[data-bs-dismiss="modal"]'
  );
  if (yesButton) {
    await yesButton.click();
  } else {
    console.warn('Could not find the "Yes" button to confirm deletion.');
  }
  await page.waitForTimeout(250);

  return (await page.$$('a[href^="/dashboard/"]')).length > 1;
};
...
const createMonitor = async (page: Page, monitor: UptimeRobotMonitor) => {
  ...
  switch (monitor.type) {
    ...
    default:
      console.log(
        `Monitor type ${monitor.type} of ${monitor.friendly_name} is not supported`
      );
      break;
  }

  await page
    .getByLabel("Heartbeat Interval (Check every")
    .fill(String(monitor.interval));
...
const deleteKumaMonitors = async () => {
  const browser = await startPlaywright();
  const page = await browser.newPage();
  await page.goto(KUMA_URL);
  ...
  var hasMore = true;
  while (hasMore) {
    hasMore = await deleteKumaMonitorOnPage(page);
  }
